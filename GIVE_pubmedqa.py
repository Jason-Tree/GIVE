...
if __name__ == "__main__":
    ...
    question_pubmed, key_pubmed = load_pubmedQA()
    question_key = list(key_pubmed.keys())
    answers_first = {}
    answers_second = {}
    answers_third = {}
    temperature = args.temperature
    entity_per_group = args.entity_per_group
    for key_idx in tqdm(range(len(question_key))):
        key = question_key[key_idx]
        Q = question_pubmed[key]['QUESTION']
        ...
        for i in range(len(groups)):
            ...
            for j in range(i + 1, len(groups)):
                group2 = groups[j]
                yes_candidates, maybe_candidates, no_candidates, self_candidates, KG_candidates = build_connections(Q_rewrite,
                                                                                                                    group1,
                                                                                                                    group2,
                                                                                                                    relationship_list,
                                                                                                                    model,
        ...
        answer_first = generate_answer_a_pubmedqa(Q_rewrite, affirmative_txt, temperature, model)
        if(counterfactual_txt != ""):
            answer_second = generate_answer_ac_pubmedqa(Q_rewrite, affirmative_txt, counterfactual_txt, answer_first, temperature, model)
        else:
            answer_second = answer_first
        if(KG_txt != ""):
            answer_third = generate_answer_ace_pubmedqa(Q_rewrite, affirmative_txt, counterfactual_txt, KG_txt, answer_first, answer_second, temperature, model)
        else:
            answer_third = answer_second
        answers_first[key] = answer_first
        answers_second[key] = answer_second
        answers_third[key] = answer_third

    with open("GIVE_pubmedqa_a.json", "w") as outfile:
        json.dump(answers_first, outfile)
    with open("GIVE_pubmedqa_ac.json", "w") as outfile:
        json.dump(answers_second, outfile)
    with open("GIVE_pubmedqa_ace.json", "w") as outfile:
